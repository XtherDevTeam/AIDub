...
def generate_text_list(colab_project_prefix: pathlib.Path = pathlib.Path(config.save_dest_for_downloaded_voice).absolute()) -> list[str]:
    ...
    for char in collection:
        for i in collection[char]:
            ...
            generated += f"{colab_project_prefix / dest}|{char}|{config.muted_language}|{text.replace('\n', '')}\n"
        
            with open(pathlib.Path(config.save_dest_for_downloaded_voice) / f"{char}.list", "w+") as f:
                f.write(generated)